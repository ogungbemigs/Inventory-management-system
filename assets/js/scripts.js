...
$(document).ready(function() {
    ...
    let counter = setInterval(() => {
        $.ajax({
            url: 'model/item/deviceCount.php',
            method: 'POST',
            dataType: 'json',
            //data: formData,
            //contentType: false,
            ...
            success: function(data) {
                ...
                //alert(data)
                $("#devices").html(data.device);
                $("#os_status").html(data.os);
    ...
    $('#staffName').keyup(function() {
        showSuggestions('staffName', showItemNamesFile, 'staffNameSuggestionsDiv');
    });

    // Remove the item names suggestions dropdown in the item details tab
    // when user selects an item from it
    $(document).on('click', '#staffNamesSuggestionsList li', function() {
        $('#staffName').val($(this).text());
        $('#staffNamesSuggestionsList').fadeOut();
    });

    // Listen to item number text box in item details tab
    $('#deviceCode').keyup(function() {
        showSuggestions('deviceCode', showItemNumberSuggestionsFile, 'deviceCodeSuggestionsDiv');
    });

    // Remove the item numbers suggestions dropdown in the item details tab
    // when user selects an item from it
    $(document).on('click', '#deviceCodeSuggestionsList li', function() {
        $('#deviceCode').val($(this).text());
        $('#deviceCodeSuggestionsList').fadeOut();
...
function addItem() {
    ...
    var deviceRam = $('#deviceRam').val();
    var deviceStorage = $('#deviceStorage').val();
    var os = $('#os').val();
    var osStatus = $('#osStatus').val();
    var macAddress = $('#macAddress').val();
    var esetStatus = $('#esetStatus').val();
    var activeDirectory = $('#activeDirectory').val();
    ...
    $.ajax({
        ...
        complete: function() {
            ...
            getItemStockToPopulate('deviceCode', getItemStockFile, itemDetailsTotalStock);
            searchTableCreator('itemDetailsTableDiv', itemDetailsSearchTableCreatorFile, 'itemDetailsTable');
            reportsTableCreator('itemReportsTableDiv', itemReportsSearchTableCreatorFile, 'itemReportsTable');
        }
    });
...
function getItemDetailsToPopulate() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#deviceRam').val(data.device_ram).trigger("chosen:updated");
            $('#deviceStorage').val(data.device_storage).trigger("chosen:updated");
            $('#os').val(data.os).trigger("chosen:updated");
            $('#osStatus').val(data.os_status).trigger("chosen:updated");
            $('#macAddress').val(data.mac_address);
            $('#esetStatus').val(data.eset_status).trigger("chosen:updated");
            $('#activeDirectory').val(data.ad_status).trigger("chosen:updated");
            $('#printer').val(data.printer);
            $('#scanner').val(data.scanner);
            $('#itemDetailsStatus').val(data.status).trigger("chosen:updated");


            newImgUrl = 'data/item_images/' + data.device_code + '/' + data.imageURL;
...
function getItemName(itemNumberTextBoxID, scriptPath, itemNameTextbox) {
    // Get the itemNumber entered in the text box
    var deviceCode = $('#' + itemNumberTextBoxID).val();
